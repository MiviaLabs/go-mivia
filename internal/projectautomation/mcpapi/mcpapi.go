...
func ToolDefinitions() []map[string]any {
	...
	return []map[string]any{
		...
		tool("projects.automations.create_remediation_from_finding", "Create Remediation From Finding", "Creates a governed remediation Work Plan, ready Work Task, and enabled automatic automation from confirmed finding metadata only. Use only after independent review confirms a real bug. Generated remediation tasks require a focused regression test when feasible, or a concrete not-feasible reason in the task outcome. For implementation work, set likely_files_affected to safe directory scopes for module wiring, new services, tests, and generated artifacts; the service expands files_to_edit from those safe scopes for GitOps. Safety: refs and bounded summaries only; no raw prompts, source, stderr, provider payloads, secrets, roots, external URLs, or PII. If activate_plan is true, the Work Plan status trigger queues execution; do not call projects.automations.run for normal flow.", schema(map[string]any{"id": ref, "project_id": ref, "finding_ref": ref, "finding_status": map[string]any{"type": "string", "enum": []string{"confirmed"}}, "title": text, "summary": map[string]any{"type": "string", "minLength": 1, "maxLength": 500}, "severity": ref, "owner_agent": ref, "implementation_agent_id": ref, "created_by_run_id": ref, "trace_id": ref, "permission_snapshot_ref": ref, "git_base_ref": ref, "git_branch_ref": ref, "git_worktree_ref": ref, "files_to_read": map[string]any{"type": "array", "items": map[string]any{"type": "string", "minLength": 1, "maxLength": 300}, "maxItems": 100}, "files_to_edit": map[string]any{"type": "array", "items": map[string]any{"type": "string", "minLength": 1, "maxLength": 300}, "maxItems": 100}, "likely_files_affected": map[string]any{"type": "array", "items": map[string]any{"type": "string", "minLength": 1, "maxLength": 300}, "maxItems": 100}, "evidence_refs": refArray, "verification_requirement": map[string]any{"type": "string", "minLength": 1, "maxLength": 500}, "review_gate": map[string]any{"type": "string", "maxLength": 500}, "activate_plan": map[string]any{"type": "boolean"}}, []string{"id", "finding_ref", "finding_status", "title", "summary", "verification_requirement"})),
