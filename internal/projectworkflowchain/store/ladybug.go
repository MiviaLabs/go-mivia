...
func chainRunNode(run projectworkflowchain.ChainRun) ladybug.Node {
	...
	return ladybug.Node{Label: labelProjectWorkflowChainRun, ID: graphID(run.ProjectID, run.ID), Properties: map[string]string{
		...
		"gitops_attempt_count":         strconv.Itoa(run.GitOpsAttemptCount),
		"gitops_failure_category":      run.GitOpsFailureCategory,
		"gitops_failure_evidence_refs": joinList(run.GitOpsFailureEvidenceRefs),
		"gitops_recovery_status":       run.GitOpsRecoveryStatus,
...
func nodeToChainRun(node ladybug.Node) projectworkflowchain.ChainRun {
	...
	gitOpsAttemptCount, _ := strconv.Atoi(props["gitops_attempt_count"])
	return projectworkflowchain.ChainRun{
		...
		GitOpsAttemptCount:        gitOpsAttemptCount,
		GitOpsFailureCategory:     props["gitops_failure_category"],
		GitOpsFailureEvidenceRefs: splitList(props["gitops_failure_evidence_refs"]),
		GitOpsRecoveryStatus:      props["gitops_recovery_status"],
