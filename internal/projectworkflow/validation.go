...
func (v *workflowValidator) validateSteps(agents []WorkflowAgentDefinition, steps []WorkflowStep) {
	...
	for i, step := range steps {
		...
		v.textList(step.AcceptanceCriteria, base+".acceptance_criteria", 300)
		v.textList(step.StopConditions, base+".stop_conditions", 300)
		v.textList(step.VerifierLadder, base+".verifier_ladder", 300)
		v.optionalText(step.RegressionApplicability, base+".regression_test_applicability", 300)
		v.refList(step.DownstreamImpactRefs, base+".downstream_impact_refs")
		v.optionalText(step.OutputContract, base+".output_contract", 500)
