...
func ToolDefinitions() []map[string]any {
	...
	return []map[string]any{
		...
		tool("projects.agent_definitions.list", "List Workflow Agent Definitions", "MUST inspect bounded agent definitions before workflow automation or runner configuration. Work Plans and Work Tasks remain mandatory, and permission snapshots must gate downstream automation metadata. Safety: metadata-only instructions, skills/tools/command policy; no raw prompts/source/logs/provider payloads/secrets/roots/PII.", schema(merge(map[string]any{"id": ref, "workflow_id": ref}, pageFields), []string{"id", "workflow_id"})),
		tool("projects.agent_definitions.get", "Get Workflow Agent Definition", "MUST inspect one bounded workflow agent definition before assigning Work Tasks or automation. Review gates and independent review remain required where relevant. Safety: metadata-only instructions and permission policy; no raw prompts/source/logs/provider payloads/secrets/roots/PII.", schema(map[string]any{"id": ref, "workflow_id": ref, "agent_id": ref}, []string{"id", "workflow_id", "agent_id"})),
