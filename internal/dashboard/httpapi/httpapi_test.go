...
func TestRoutes_DashboardServesEmbeddedAssets(t *testing.T) {
	...
	for _, want := range []string{"contextHealthPill", "projectValidationPill", "project enabled", "config ${status}", "latest run ${status}", "EventSource", "agent-activity/stream", "Call summary", "Full payload", "visibleActivityRows", "policy_category", "grouped_event_count", "Context-pack manifest", "manifest only"} {
