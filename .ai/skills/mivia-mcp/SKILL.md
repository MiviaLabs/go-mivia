...
- Exception: when an automation runner prompt says this is a dedicated worktree run, MCP workspace edit tools still target the canonical base checkout unless the tool explicitly exposes a dedicated-worktree edit target. In that case, do source edits in the current process workspace provided by the runner, and use MCP only for bounded evidence, claims, verifier refs, and Work Task status updates.
...
- Before rerunning a live workflow chain after a pipeline bug, review downstream boundaries that can consume the fixed artifact and add or identify tests for their expected artifact shapes and blocking behavior. Do not use live observation as the first test for a foreseeable downstream contract failure.
...
10. Attach independent review result refs with `projects.work_tasks.attach_review_result` before completing any non-trivial or write-capable task. Review refs must be short safe identifiers such as `review-workplan-mcp`. The reviewer must be a different agent/run from the implementing `claimed_by_run_id` when known. Use `review_exempt_reason` only for tiny mechanical tasks with no code, config, data, auth, tenancy, privacy, API, migration, automation, or promotion risk.
   - Do not depend on Codex Desktop subagent/thread capacity for this gate. Independent review is a Work Task/automation responsibility in Mivia metadata. If a UI client cannot spawn a fresh reviewer thread, claim or queue the reviewer Work Task through Mivia automation, reuse an already independent reviewer run, or block with `reviewer_capacity_unavailable`; do not weaken the review gate or self-review.
...
Workspace tools are default-disabled and require both global `[workspace].enabled = true` and per-project `workspace_mode = "read_only"` or `"edit"` with `digest_mode = "content_graph"`. `read_only` allows governed git status/diff and current eligible file reads. `edit` additionally allows exact byte-span edits and eligible single-file deletes guarded by an opaque per-process token from `projects.workspace.file_read`, plus new eligible text-file creation through `projects.workspace.file_create`; use these paths before shell, `apply_patch`, or manual file operations when the file is eligible. File reads return full eligible text unless the caller passes an explicit positive `max_bytes` cap. There is no arbitrary shell endpoint, public exposure, auth change, provider call, embedding/vector/crawling path, raw DB query endpoint, raw patch upload endpoint, recursive delete, or direct MCP git commit/push/checkout/reset/branch/merge/rebase/stash/clean/restore tool.

Git commit, push, and draft PR creation are available only through configured automation runner GitOps, not as direct MCP workspace tools. When `[git_operations]` is enabled for a supervised external `mivia-automation-runner`, the runner may reconcile a successful task attempt before reporting it: inspect the `--codex-cd` worktree, commit task changes, optionally push using the configured SSH key and known_hosts paths, and optionally create or reuse a draft GitHub PR through `gh`. Post-task commit fails closed unless the Work Task supplies at least one safe scoped affected path; the runner does not stage the whole worktree as a fallback. Public key paths are validation metadata only; push requires a private key plus known_hosts. GitHub auth must be an env or file reference. Never place key contents, token literals, configured secret paths, raw command output, raw stderr, roots, prompts, source dumps, or PII in MCP metadata.
