...
- Workflow TOML is compile-only metadata. Use `projects.workflows.validate_toml` and `projects.workflows.import_toml` to validate and store workflow definitions, agent definitions, bounded agent instructions, review gates, dependencies, and permission snapshots; then use `projects.workflows.compile_to_work_plan` to create Work Plan, Work Task, reviewer task, automation, and permission snapshot refs. TOML import does not execute anything, automation cannot bypass Work Plans/Tasks, agent instructions are operational metadata rather than raw prompts, and permission snapshots are policy metadata, not OS sandbox proof.
