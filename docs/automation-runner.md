...
Set `MIVIA_AUTOMATION_UID="$(id -u)"` and `MIVIA_AUTOMATION_GID="$(id -g)"` when the host checkout is not owned by that default UID and GID. Do this before enabling commit, push, or draft PR automation. Configure `MIVIA_CONTAINER_USER` separately when the server needs different permissions for its data volume or local workspace mounts.
...
The helper exports `MIVIA_AUTOMATION_UID="$(id -u)"` and `MIVIA_AUTOMATION_GID="$(id -g)"` unless you already set them, includes `.docker-compose.local.yml` when present, and then runs `docker compose up` with the repository compose files. It does not override `MIVIA_CONTAINER_USER`; the server may need a different user for its data volume or local workspace mounts. Pass normal `docker compose up` flags after the script name.
...
Set `MIVIA_AUTOMATION_UID="$(id -u)"` and `MIVIA_AUTOMATION_GID="$(id -g)"` for the automation sidecar. Avoid `0:0`; root-run sidecars create root-owned commits, refs, and worktree metadata on Linux and macOS bind mounts.
