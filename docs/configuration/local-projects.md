...
Do not treat that as a Go module proxy path. Go module publication uses repository semantic-version tags such as `v0.1.5`; container publication uses registry image tags such as `0.1.5` when the release workflow chooses that tag.
