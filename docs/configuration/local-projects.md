...
Do not treat that as a Go module proxy path. Go module publication uses repository semantic-version tags such as `v0.2.3`; container publication uses registry image tags such as `0.2.3` when the release workflow chooses that tag.
