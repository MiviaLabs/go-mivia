...
```yaml
# image: ghcr.io/mivialabs/go-mivia:0.1.6
```

Do not treat that as a Go module proxy path. Go module publication uses repository semantic-version tags such as `v0.1.6`; container publication uses registry image tags such as `0.1.6` when the release workflow chooses that tag.
